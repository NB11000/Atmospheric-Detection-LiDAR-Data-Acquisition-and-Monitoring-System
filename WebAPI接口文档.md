...
目前检测逻辑中,数据采集子进程中的检测线程已经做好了站位处理。也根据g r p c发布到 主控进程的g r p c服务中 但目前主控进程缺少一个服务来接收这个检测的数据并且把这个检测的数据发布到m q t t主题上。我希望这个服务可以和之前的另外，3个低频数据分发服务数据持久化服务和高频波形发布服务一样。开始分析，这个需求，并制定一个方案 
