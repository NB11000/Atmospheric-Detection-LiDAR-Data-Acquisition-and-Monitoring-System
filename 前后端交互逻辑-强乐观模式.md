...
sequenceDiagram
    ...
    M->>SS: UpdateCollectorState / ResetCollectorState
    SS->>SS: 更新 volatile 状态缓存
    M->>EP: PublishStateChangedAsync(eventType, ...)
    EP->>EP: 构建 StateChangedEvent (含全量 SystemStateDto)
...
- **执行后**：`CniLaser` 调用 `SystemStateService.UpdateLaserState()` 更新内存缓存。
...
- **服务端处理**：
  - 状态变更事件到达 `SystemStateService`，更新 volatile 缓存。
  - `MqttEventPublisher.PublishStateChangedAsync()` 构建 `StateChangedEvent`（含全量 `SystemStateDto`），发布至 `daq/{MachineId}/events/state_changed`，QoS=1。
...
5. **最终一致性**：接受极短时间内（1-2 秒，遗嘱场景下最多 90 秒）前端乐观状态与真实状态可能的不一致，以换取系统整体性能与简洁性。
6. **协议统一化**：命令下发和状态推送统一走 MQTT，消除 HTTP REST / SignalR / gRPC 多协议维护负担。SignalR 通道降级为兼容层（仅 `SignalRHubPublisher` 向后兼容，不作为主通道）。
7. **状态推断而非轮询**：主控进程不主动轮询子进程状态，通过对 command_response 的解析推断状态变化，减少网络开销。
...
function applyStateToUI(state: SystemStateDto) {
  ...
  // 按钮可用性（完全信任 uiHints）
  setButtonEnabled('openCollector', state.uiHints.canOpenCollector);
  setButtonEnabled('closeCollector', state.uiHints.canCloseCollector);
  setButtonEnabled('startAcquisition', state.uiHints.canStartAcquisition);
  setButtonEnabled('stopAcquisition', state.uiHints.canStopAcquisition);
  setButtonEnabled('connectLaser', state.uiHints.canConnectLaser);
  setButtonEnabled('disconnectLaser', state.uiHints.canDisconnectLaser);
  setButtonEnabled('laserOn', state.uiHints.canTurnLaserOn);
  setButtonEnabled('laserOff', state.uiHints.canTurnLaserOff);
}
```
...
private static async Task<byte[]> HandleLaserOn(byte[] _)
{
    ...
    // 3. CniLaser 内部已调用 UpdateLaserState()
    // ⚠️ MQTT state_changed 推送尚未实现，见文档缺口说明
...
| **采集子进程 gRPC 断连** | RPC 返回成功（乐观更新），随后收到 `state_changed`（`collector_disconnected`）纠偏 | `GrpcServiceImpl` 检测流断连 → `ResetCollectorState` → 发布 `state_changed` | 1-2 秒延迟 |
| **多客户端操作冲突**（A 开/B 关） | 各自乐观更新后，都收到 `state_changed` 广播，同步为最终状态 | 基于缓存序列化处理（MQTT 消息有序到达），广播最终状态 | 快速达到最终一致 |

## 激光状态推送缺口

> **当前实现状态（2026-05）**：`CniLaser.cs` 中所有 MQTT `PublishStateChangedAsync()` 调用已被注释掉。激光器的连接/断开/开启/关闭操作**不会触发 `state_changed` 事件推送**，仅更新 `SystemStateService` 内存缓存。
...
**待完成**：
- 恢复 `CniLaser.cs` 中的 `_mqttEventPublisher.PublishStateChangedAsync(...)` 调用，在 `Connect` / `Disconnect` / `LaserOn` / `LaserOff` 成功后发布对应 `eventType`（`laser_connected` / `laser_disconnected` / `laser_emission_on` / `laser_emission_off`）。
...
| `daq/{id}/events/state_changed` | 1 | 状态变更纠偏（全量 SystemStateDto） | 无条件覆盖 UI 状态 + 按钮启用/禁用 |
